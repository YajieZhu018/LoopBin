...
name_epi = ["_CTCF2K.bedgraph", "_H3K27ac2K.bedgraph",
            "_H3K27me32K.bedgraph", "_SMC1A2K.bedgraph"]

reso_micro_c = 8000  # kb
size_micro_c = 7  # bins of 8kb reso
reso_epic = 2000
size_epic = 12  # bins 0f 2kb reso
...
def calcualte_micro_c_submatrix_region_with_loop_midpoint(midpoint):
    """
    ...
    """
    start = reso_micro_c * (midpoint // reso_micro_c - size_micro_c // 2)
    end = start + reso_micro_c * size_micro_c
    return int(start), int(end)

def calcualte_epic_submatrix_region_with_loop_midpoint(midpoint):
    """
    Calculate the start and end positions of a submatrix of epics based on the loop
    anchor midpoint.
    ...
    Returns:
        tuple: The start and end positions of the epic submatrix as integers.
    """
    start = reso_micro_c * (midpoint // reso_micro_c) - reso_epic * (size_epic // 3)
    end = start + reso_epic * size_epic
    return int(start), int(end)
...
def cool_to_matrix(hic, first_anchor, second_anchor):
    ...
    nan_cutoff = size_micro_c * size_micro_c / 2
...
def extract_signal_from_bedgraph(name_file, chrom, start, end):
    ...
    starting = int(start / reso_epic)
    ending = int(end / reso_epic) - 1
...
def merge_matrices_to_multichannel_image(matrix_list):
    ...
    height = size_epic
    width = size_epic
    multichannel_matrix = np.zeros((height, width, 4), dtype='float')
...
def task(hic, first_anchor_micro_c, second_anchor_micro_c, first_anchor_epic, second_anchor_epic, name_file, i):
    """
    ...
    Returns:
        ...
        None otherwise.
    """
    chrom1, start1, end1 = first_anchor_epic
    chrom2, start2, end2 = second_anchor_epic
    mat = cool_to_matrix(hic, first_anchor_micro_c, second_anchor_micro_c)
    if mat is None:
...
def input_creator_r(loop_file, cool_file, bedgraph, n_cpu):
    ...
    with multiprocessing.Pool(n_cpu) as pool:
        ...
        for loop in read_file(loop_file):
            ...
            start1_micro_c, end1_micro_c = calcualte_micro_c_submatrix_region_with_loop_midpoint(
                midpoint1)
            start2_micro_c, end2_micro_c = calcualte_micro_c_submatrix_region_with_loop_midpoint(
                midpoint2)
            # get the position of epic
            start1_epic, end1_epic = calcualte_epic_submatrix_region_with_loop_midpoint(
                midpoint1)
            start2_epic, end2_epic = calcualte_epic_submatrix_region_with_loop_midpoint(
                midpoint2)
            # Filtering the position
            ranges1 = [(start1_micro_c, end1_micro_c), (start1_micro_c - reso_micro_c, end1_micro_c - reso_micro_c),
                       (start1_micro_c + reso_micro_c, end1_micro_c + reso_micro_c), (start1_micro_c - reso_micro_c * 2, end1_micro_c -
                                                      reso_micro_c * 2), (start1_micro_c + reso_micro_c
                                                                  * 2, end1_micro_c +
                                                                  reso_micro_c * 2)]
            for start, end in ranges1:
                intersected = list(set(range(start, end, reso_micro_c)).intersection(
                    range(start2_micro_c, end2_micro_c, reso_micro_c)))
                if intersected:
                    break
            if intersected:
                continue
            ranges2 = [(start2_micro_c, end2_micro_c), (start2_micro_c - reso_micro_c, end2_micro_c - reso_micro_c),
                       (start2_micro_c + reso_micro_c, end2_micro_c + reso_micro_c),
                       (start2_micro_c - reso_micro_c * 2, end2_micro_c - reso_micro_c * 2),
                       (start2_micro_c + reso_micro_c * 2, end2_micro_c + reso_micro_c * 2)]
            for start, end in ranges2:
                intersected = list(set(range(start1_micro_c, end1_micro_c, reso_micro_c)).intersection(
                    range(start, end, reso_micro_c)))
                if intersected:
                    break
            if intersected:
                continue
            result = pool.apply_async(task, (hic, [chrom1, start1_micro_c, end1_micro_c],
                                             [chrom2, start2_micro_c, end2_micro_c],
                                             [chrom1, start1_epic, end1_epic],
                                            [chrom2, start2_epic, end2_epic],
                                             name_file, i))
...
def mise_en_forme(data_filtered):
    ...
    """
    epigenetic = np.ones((len(data_filtered), size_epic * 2, 4))
    micro_c = np.ones((len(data_filtered), size_micro_c * size_micro_c))
...
def save_log(epigenetic, micro_c, dir_path):
    ...
    Args:
        ...
        micro_c (numpy.ndarray): The log-transformed micro_c data.splitting
...
def log_epi(sorted_epigenetic):
    ...
    ctcf = np.reshape(ctcf, (len(sorted_epigenetic), size_epic * 2))
    hac = np.reshape(hac, (len(sorted_epigenetic), size_epic * 2))
    hme = np.reshape(hme, (len(sorted_epigenetic), size_epic * 2))
    smc1 = np.reshape(smc1, (len(sorted_epigenetic), size_epic * 2))
...
def create_data(log_epigenetic, log_micro_c):
    ...
    """
    mem_h3ac = np.empty((log_epigenetic.shape[0], size_epic, size_epic))
    mem_h3me = np.empty((log_epigenetic.shape[0], size_epic, size_epic))
    mem_smc1 = np.empty((log_epigenetic.shape[0], size_epic, size_epic))
    mem_ctcf = np.empty((log_epigenetic.shape[0], size_epic, size_epic))
    log_ct, log_hac, log_hme, log_sm = splitting(log_epigenetic, 1)
    # give the epigenetics data as matrix
    for nbr, i in enumerate(log_ct):
        mem_ctcf[nbr] = np.outer(i[size_epic:], i[:size_epic])
        mem_h3ac[nbr] = np.outer(log_hac[nbr][size_epic:], log_hac[nbr][:size_epic])
        mem_h3me[nbr] = np.outer(log_hme[nbr][size_epic:], log_hme[nbr][:size_epic])
        mem_smc1[nbr] = np.outer(log_sm[nbr][size_epic:], log_sm[nbr][:size_epic])

    log_micro_c = np.reshape(log_micro_c, (log_ct.shape[0], size_micro_c, size_micro_c))
    multichannel_matrix = np.zeros((log_ct.shape[0],  size_epic, size_epic, 4),
                                   dtype='float')
    for i in range(log_ct.shape[0]):
        multichannel_matrix[i] = merge_matrices_to_multichannel_image(
            [mem_ctcf[i], mem_h3ac[i], mem_h3me[i],
             mem_smc1[i]])  # , log_micro_c[i]])
    return multichannel_matrix
...
def process(loop_file, hic, bedgraph, nbr_cpu, folder):
    ...
    if folder is not None and os.path.exists(folder):
        dir_path = f"{folder}/save"
        name = f"{folder}/save/loop_file_analysis.bedpe"
    else:
        dir_path = "save"
        name = "save/loop_file_analysis.bedpe"
    ...
    np.save(f"{dir_path}/epic_data.npy", multichannel_matrix)
    # generate merged data 
    merged = np.concatenate((log_micro_c, log_epigenetic), axis=1)
    np.save(f"{dir_path}/merged_data.npy", merged)

    print(f"Data \"process_data.npy\" ready to use in {dir_path}")
